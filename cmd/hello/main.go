...
func (h *Handlers) PostHello(c echo.Context) error {
	...
	// Передаём указатель для привязки JSON
	if err := c.Bind(input); err != nil {
		return c.String(http.StatusBadRequest, "Ошибка при обработке JSON: "+err.Error())
	}

	// Сохраняем сообщение в базу данных
	if err := h.dbProvider.InsertHello(input.Msg); err != nil {
		return c.String(http.StatusInternalServerError, "Ошибка сохранения в БД: "+err.Error())
	}
